...
J =  1/m*sum(sum(-yi.*log(a3)-((1-yi).*log(1-a3))))+...
    lambda/2/m*(sum(sum(Theta1_1.^2))+sum(sum(Theta2_1.^2)));
% for i = 1:num_labels    
%     J = J + 1/m*sum(-yi(:,i).*log(a3(:,i))-(1-yi(:,i)).*log(1-a3(:,i)));
%         %lambda/2/m*sum(theta_1.^2); %这里是总的代价函数 应该是一个标量
% end
